...
const Hints = Module("hints", {
     ...
     */
    _generate: function (win, screen, allHeight, allWidth) {
        if (!win)
            win = config.browser.contentWindow;
        if (!screen)
        	screen = {top: 0, left: 0, bottom: !allHeight ? win.innerHeight : win.height, right: !allWidth ? win.innerWidth : win.width};
        ...
        for (let frame in util.Array.itervalues(win.frames)) {
            ...
            this._generate(frame,{
                ...
                bottom : Math.min(!allHeight ? frame.innerHeight : frame.height, screen.bottom - rect.top),
                right  : Math.min(!allWidth ? frame.innerWidth : frame.width,  screen.right  - rect.left)
              });
        }
     ...
     */
    show: function (minor, filter, win, allHeight, allWidth) {
        ...
        this._generate(win, undefined, allHeight, allWidth);
